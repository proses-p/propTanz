...
import { useLocation, useNavigate } from "react-router-dom"; // Fixed: Added imports
...
export default function HostelManagement() {
    ...
    // NOTE: Inabidi u-fetch data kutoka kwenye API yako hapa ndani ya loadHostels badala ya kubadilisha tu currentPage state.
    const loadHostels = async (page = 1) => {
        try {
            setCurrentPage(page);
            // Mfano wa ku-fetch data:
    ...
    // Fixed: Added handleUpdateHostel function
    ...
    // Fixed: Added 'data' parameter to runAction if you intend to use it
    const runAction = async (action, successMessage, data) => {
        try {
            await hostelService.update(selectedHostel.id, data);
            await loadHostels();
    ...
    return (
        <div className="mx-auto max-w-[1500px]">
            <div className="mb-9 flex flex-col gap-5 border-b border-[#eee6c7] pb-8 sm:flex-row sm:items-end sm:justify-between">
                ...
                <button onClick={() => { setIsEditing(false); setSelectedHostel(null); setOpenModal(true); }} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-[#FDBF2D] px-5 py-3 font-bold text-slate-950 shadow-[0_10px_22px_rgba(253,191,45,0.24)] transition hover:bg-[#FAF92A]">
            ...
            {sortedHostels.length > 0 ? (
                <HostelTable hostels={sortedHostels} onEdit={handleEditClick} onDelete={(hostel) => { setHostelToDelete(hostel); setDeleteModalOpen(true); }} />
            ) : (
                <EmptyState title="No hostels found" message="There are no hostels matching your search." />
            )}
            
            <Pagination currentPage={currentPage} lastPage={pagination.lastPage || 1} onPageChange={loadHostels} />
